...
public class AudioTool {
    ...
    /**
     * @param audios             audio that will be mix
     * @param outputPath         output audio file
     * @param onCompleteCallback lambda with result audio
     */
    public  AudioTool mixAudios(String[] audios,Double pitchConfig,List<Double> volumeConfig,List<Double> panPlayerConfig,String outputPath, OnFileComplete onCompleteCallback) throws IOException{
        FileManager.deleteFile(outputPath);
        FileManager.validateOutputFile(outputPath);

        char[] pathArray = "abcdefghijklmnopqrstuvwxyz".toCharArray();
        StringBuilder mixPath = new StringBuilder();
        String atempo = "";
        StringBuilder volumeStr = new StringBuilder().append('"');
        StringBuilder endStr = new StringBuilder();
        int index = 0;
        for(String path : audios) {
            String pan = "pan=stereo|c0=c0|c1=c1";
            if(panPlayerConfig.get(index) > 0){
                pan = "pan=stereo|c0="+(1.0-(panPlayerConfig.get(index)/100))+"*c0|c1="+panPlayerConfig.get(index)/100+"*c1";
            }else if(panPlayerConfig.get(index) < 0){
                pan = "pan=stereo|c0="+Math.abs(panPlayerConfig.get(index)/100)+"*c0|c1="+(1.0-Math.abs(panPlayerConfig.get(index)/100))+"c1";
            }
            if(pitchConfig > 0) {
                atempo = "asetrate=88200,aresample=44100,atempo=" + ((0.1 * pitchConfig) + 0.5);
            }else  if(pitchConfig < 0){
                atempo = "asetrate=22050,aresample=44100,atempo=" + ((Math.abs(pitchConfig)*0.1)+ 1.5);
            }


            mixPath.append(" -i ").append(path);
            volumeStr.append("["+index+"]volume="+((volumeConfig.get(index)/100)*4)+","+pan+","+atempo+"["+pathArray[index]+"];");
            endStr.append("["+pathArray[index]+"]");

            index++;
        }
        String command = String.format(Locale.US,
                "%s -filter_complex %s%samix=inputs=%d:duration=longest"+'"'+"  %s",
                mixPath,volumeStr,endStr,audios.length,outputPath
        );
